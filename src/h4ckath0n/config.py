...
class Settings(BaseSettings):
    ...
    jobs_default_queue: str = Field(default="default", description="Default job queue name")

    # --- Storage ---
    storage_backend: str = Field(default="local", description="Storage backend (`local` or `s3`)")
    storage_dir: str = Field(
        default="./.h4ckath0n_storage", description="Local storage directory path"
    )
    ...
    email_outbox_dir: str = Field(
        default="./.h4ckath0n_email_outbox", description="Directory for the file email backend"
    )
    ...
    # --- Demo ---
    demo_mode: bool = Field(default=False, description="Enable demo mode features")
