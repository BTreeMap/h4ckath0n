...
class Settings(BaseSettings):
    ...
    jobs_inline_in_dev: bool = Field(
        default=True, description="Run background jobs inline when in development mode"
    )
    ...
    # --- Storage ---
    storage_backend: str = Field(default="local", description="Storage backend (`local` or other)")
    ...
    email_outbox_dir: str = Field(
        default="./.h4ckath0n_email_outbox", description="Directory for local email outbox"
    )
    ...
    # --- Demo ---
    demo_mode: bool = Field(default=False, description="Enable read-only demo mode features")
