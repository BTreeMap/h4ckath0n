...
class Settings(BaseSettings):
    ...
    jobs_inline_in_dev: bool = Field(
        default=True, description="Run jobs inline in development instead of using the worker"
    )
    ...
    app_base_url: str = Field(
        ...
        description="Base URL of the frontend app (used for emails)",
    )
    ...
    smtp_host: str = Field(default="", description="SMTP host for email backend")
    smtp_port: int = Field(default=587, description="SMTP port for email backend")
    smtp_username: str = Field(default="", description="SMTP username for email backend")
    smtp_password: str = Field(default="", description="SMTP password for email backend")
    smtp_starttls: bool = Field(default=True, description="Use STARTTLS for SMTP email backend")
    smtp_ssl: bool = Field(default=False, description="Use SSL for SMTP email backend")

    # --- Demo ---
    demo_mode: bool = Field(
        default=False, description="Enable demo mode (disables some destructive actions)"
    )
