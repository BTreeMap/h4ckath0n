...
class Settings(BaseSettings):
    ...
    jobs_inline_in_dev: bool = Field(
        default=True, description="Run jobs synchronously in development"
    )
    ...
    storage_backend: StorageBackend = Field(
        default="local", description="Storage backend ('local' only currently)"
    )
    storage_dir: str = Field(
        default="./.h4ckath0n_storage", description="Directory for local file storage"
    )
    max_upload_bytes: int = Field(
        default=50 * 1024 * 1024, description="Maximum allowed file upload size in bytes"
    )
    ...
    email_backend: EmailBackend = Field(default="file", description="'file' or 'smtp'")
    ...
    demo_mode: bool = Field(
        default=False, description="Enable demo mode constraints (read-only profiles, etc.)"
    )
