...
class Settings(BaseSettings):
    ...
    jobs_inline_in_dev: bool = Field(
        default=True, description="Run jobs inline instead of enqueueing in development mode"
    )
    ...
    email_from: str = Field(
        default="noreply@localhost", description="Default from address for outgoing emails"
    )
    email_outbox_dir: str = Field(
        default="./.h4ckath0n_email_outbox",
        description="Directory for file-based email backend",
    )
    ...
    # --- Demo ---
    demo_mode: bool = Field(default=False, description="Enable demo mode (read-only operations)")
