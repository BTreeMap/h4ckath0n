...
def _generate_prefixed_id(prefix: str) -> str:
    """Generate a 32-character base32 ID with the given prefix."""
    s = random_base32()
    return prefix + s[1:]


def _is_valid_id(value: str, prefix: str) -> bool:
    """Return True if value is a valid 32-character base32 ID with the given prefix."""
    return (
        len(value) == _ID_LEN
        and value[:1] == prefix
