...
async def _is_bootstrap_admin(email: str, settings: Settings, db: AsyncSession) -> bool:
    ...
    if settings.first_user_is_admin:
        # ⚡ Bolt: use exists/limit(1) check instead of full table count
...
async def register_user(
    ...
    *,
    ...
    # ⚡ Bolt: select only ID to avoid hydrating full User instance for existence check
...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    ...
    # ⚡ Bolt: prefer scalar() to avoid intermediate ExecutionResult allocation
