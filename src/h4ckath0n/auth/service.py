...
async def _is_bootstrap_admin(email: str, settings: Settings, db: AsyncSession) -> bool:
    ...
    if settings.first_user_is_admin:
        # ⚡ Bolt: Query limit(1) instead of count() to prevent full table scans
...
async def register_user(
    ...
    *,
    ...
    # ⚡ Bolt: Use scalar() on ID to avoid full ORM object fetching allocations
    existing_user = await db.scalar(
        select(User.id).filter(User.email == email).limit(1)
    )
    if existing_user:
