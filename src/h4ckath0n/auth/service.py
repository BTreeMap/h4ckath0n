...
def _require_password_extra() -> tuple:  # type: ignore[type-arg]
    ...
    try:
        ...
        return hash_password, verify_password, DUMMY_HASH
...
async def register_user(
    ...
    *,
    ...
    hash_password, _verify, _dummy = _require_password_extra()
...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    _hash, verify_password, dummy_hash = _require_password_extra()
    result = await db.execute(select(User).filter(User.email == email))
