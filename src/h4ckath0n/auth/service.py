...
async def _is_bootstrap_admin(email: str, settings: Settings, db: AsyncSession) -> bool:
    ...
    if email in settings.bootstrap_admin_emails:
        return True
    # ⚡ Bolt: Use limit(1) instead of expensive full-table func.count()
    return bool(
...
async def register_user(
    ...
    *,
    ...
    # ⚡ Bolt: Use db.scalar(select(User.id)) instead of full ORM fetch for existence check
...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    ...
    # ⚡ Bolt: Use db.scalar() to avoid ExecutionResult allocation overhead
