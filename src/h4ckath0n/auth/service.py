...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    ...
    # Dummy hash to prevent user enumeration via timing attack.
    _dummy_hash = (
        "$argon2id$v=19$m=65536,t=3,p=4$AHof2gPKkbcCJ3Qj4wpfKA"
        "$NPqOcB+DeVtA7YbeBra5clUhi6MWGDT4hJ9pSBOzEkE"
    )
    ...
    if user is None or not user.password_hash:
        # Perform dummy verification to normalize response time
