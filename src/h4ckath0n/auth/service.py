...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    ...
    # Mitigate user enumeration via timing attacks by performing a dummy verification
    # when the user is not found or has no password hash.
    # The dummy hash must be a structurally valid Argon2id hash string.
    dummy_hash = (
        "$argon2id$v=19$m=65536,t=3,p=4$Kulkp+/MaQoaMkhYw+nvHg$"
        "EmnTTAdD8LVgRZNTFjo/xemZV9viJ3BmuaShiRsrRkc"
    )
