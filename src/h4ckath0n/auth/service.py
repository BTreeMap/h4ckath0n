...
async def _is_bootstrap_admin(email: str, settings: Settings, db: AsyncSession) -> bool:
    ...
    if settings.first_user_is_admin:
        # ⚡ Bolt: Fast existence check avoiding full table aggregate
        existing_id = await db.scalar(select(User.id).limit(1))
...
async def register_user(
    ...
    *,
    ...
    # ⚡ Bolt: Fetch only ID to avoid full ORM hydration for existence check
    existing_id = await db.scalar(select(User.id).filter(User.email == email).limit(1))
...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    ...
    # ⚡ Bolt: Use scalar() to avoid execution result parsing overhead
