...
# A pre-computed dummy Argon2id hash for constant-time processing
# Hash of 'dummy' created via: ph.hash('dummy')
_DUMMY_HASH = (
    "$argon2id$v=19$m=65536,t=3,p=4"
    "$akzVSXmRt8gmihadE06lSg"
    "$8jQQx3bYemrmsMxO8YD8aNb4FJraRshgPkceLNpq3Tk"
)
...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    ...
    # Constant-time processing to prevent user enumeration timing attacks
    if user is None or not user.password_hash:
        # Do a dummy verify so the timing is roughly the same whether the user exists or not
