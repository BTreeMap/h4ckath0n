...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    ...
    # Dummy hash to mitigate timing attacks (structurally valid Argon2id hash).
    _dummy_hash = (
        "$argon2id$v=19$m=65536,t=3,p=4$buL7sELqkygsVhMH1dETNA"
        "$uXkM8GHM85Uh9Gs/t52jc5pSVJnHfqyoIRdgZgXqHL8"
    )
    result = await db.execute(select(User).filter(User.email == email))
