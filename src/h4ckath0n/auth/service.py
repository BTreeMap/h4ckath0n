...
async def register_user(
    ...
    *,
    ...
    # ⚡ Bolt: Use scalar() with limit(1) and select(User.id) instead of full
    # query + execute().scalars().first() for existence check to prevent full
    # object parsing and overhead.
...
async def authenticate_user(db: AsyncSession, email: str, password: str) -> User | None:
    ...
    # ⚡ Bolt: Prefer scalar() over execute().scalars().first() to avoid
    # ExecutionResult allocation overhead.
