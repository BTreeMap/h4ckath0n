...
def require_scopes(*scopes: str) -> Any:
    ...
    async def _scoped(user: User = Depends(_get_current_user)) -> User:
        # Optimization: Use set(filter(None, ...)) for fast, O(1) membership testing.
        # This speeds up the 's not in user_scopes' check below, especially for users
        # with many scopes.
        user_scopes = set(filter(None, user.scopes.split(",")))
        for s in needed:
