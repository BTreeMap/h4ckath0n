...
from h4ckath0n.auth.scopes import list_scopes, normalize_scopes
...
def _cmd_users_scopes_add(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            existing = list_scopes(user.scopes)
            user.scopes = normalize_scopes(existing + args.scope)
            session.commit()
            session.refresh(user)
            _output(_user_dict(user), fmt=args.format, pretty=args.pretty)
        return EXIT_OK
...
def _cmd_users_scopes_remove(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            user.scopes = normalize_scopes(remaining)
            session.commit()
            session.refresh(user)
            _output(_user_dict(user), fmt=args.format, pretty=args.pretty)
        return EXIT_OK
