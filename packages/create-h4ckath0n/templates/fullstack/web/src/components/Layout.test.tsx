...
describe("Layout theme preference", () => {
  ...
  it("toggle from system exits system and flips effective theme", async () => {
    ...
    const button = (screen.getAllByRole("button", { name: "Theme: system (dark)" }) as HTMLElement[])[0] as HTMLElement;
  ...
  it("toggle switches only between explicit light and dark", async () => {
    ...
    const button = (screen.getAllByRole("button", { name: "Theme: light" }) as HTMLElement[])[0] as HTMLElement;
    ...
    await waitFor(() => {
      ...
      expect(document.documentElement.getAttribute("data-theme")).toBe("light");
      expect(
        screen.queryAllByRole("button", { name: /Theme: system/ })[0] || null,
