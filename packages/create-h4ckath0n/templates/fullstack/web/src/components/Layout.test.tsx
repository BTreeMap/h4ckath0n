...
describe("Layout theme preference", () => {
  ...
  it("toggle from system exits system and flips effective theme", async () => {
    ...
    const button = screen.getAllByRole("button", { name: "Theme: system (dark)" });
  ...
  it("toggle switches only between explicit light and dark", async () => {
    ...
    const button = screen.getAllByRole("button", { name: "Theme: light" });
    ...
    const buttonDark = screen.getAllByRole("button", { name: "Theme: dark" });
    fireEvent.click(buttonDark[0] as HTMLElement);
    await waitFor(() => {
      expect(localStorage.getItem("theme-preference")).toBe("light");
      expect(document.documentElement.getAttribute("data-theme")).toBe("light");
      expect(
