...
## 2023-10-25 - Avoid `func.count()` for threshold existence checks
**Learning:** `func.count()` queries force SQLAlchemy to evaluate all matching rows across the table, slowing down performance. Instead of querying for an exact length, we should use limits and check lengths (e.g., `limit(2)` and `len(active_keys.all()) <= 1`) or use `.limit(1)` and test for `is None` for pure existence checks.
**Action:** When evaluating if records meet a threshold or simply exist without needing the actual full scalar count, use `.limit(n)` directly rather than executing aggregate `func.count()`.
