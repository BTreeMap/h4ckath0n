...
## 2024-06-13 - O(1) Table Emptiness Check
**Learning:** Using select(func.count()) to check if a table is empty causes an O(N) full scan.
**Action:** Always use select(Model.id).limit(1) and check for None for an O(1) check.
