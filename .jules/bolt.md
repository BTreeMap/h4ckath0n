...
## 2026-03-21 - Avoid O(N) table count for emptiness check
**Learning:** Checking for the existence of records using `select(func.count())` forces the database to perform an expensive `O(N)` operation, which can become a bottleneck as the table grows.
**Action:** Always fetch a single ID using `select(Model.id).limit(1)` and check if the result is `None` when verifying if a table is empty.
