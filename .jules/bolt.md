...
## 2026-03-18 - Avoid O(N) count queries for existence checks

**Learning:** When checking if a table is empty or if any records exist, using `select(func.count())` forces the database to scan all matching rows (an O(N) operation), which degrades performance as the table grows.

**Action:** Always use `select(Model.id).limit(1)` and check if the result is `None` when only checking for the existence of records, turning it into an O(1) index lookup.
