...
## 2024-05-18 - Optimize Existence Checks
**Learning:** Checking for row existence using `await db.execute(select(Model))` followed by `.scalars().first()` instantiates the full ORM object which incurs unnecessary hydration overhead.
**Action:** When you only need to verify if a record exists, select a scalar column (like `Model.id`) and use `await db.scalar(select(Model.id)...)`. This bypasses the ORM hydration process and is noticeably faster, improving backend performance, particularly on frequently hit paths like user registration.
