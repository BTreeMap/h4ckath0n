...
## $(date +%Y-%m-%d) - Optimize SQLAlchemy query fetching
**Learning:** Found a specific codebase pattern where `await db.execute(select(...)).scalars().first()` and `select(func.count()).select_from(User)` were being used. These approaches result in expensive full-table queries (`func.count()`) and cause unnecessary `ExecutionResult` allocations (`execute()`) while eagerly fully-hydrating ORM models for simple existence checks.
**Action:** Applied performance optimizations replacing `execute().scalars().first()` with `scalar()`, replacing full-table `func.count()` with `.limit(1)`, and fetching only `.id` for pure existence validations. Future ORM usages should always apply `.limit(1)` and `.scalar()` instead of eager hydration via `execute` when doing simple existence lookups.
