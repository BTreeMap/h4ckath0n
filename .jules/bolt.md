...
## 2026-03-24 - Avoid count() for Table Emptiness Checks

**Learning:** When checking if a database table is empty (e.g., when determining if a user is the first user to be created), using `select(func.count()).select_from(Model)` triggers an expensive `O(N)` full table scan.
**Action:** Instead, fetch a single primary key ID using `select(Model.id).limit(1)` and check if the result is `None`. This operates in `O(1)` time by utilizing the primary key index and avoids scanning unnecessary rows.
