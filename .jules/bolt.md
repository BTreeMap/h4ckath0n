...
## 2026-03-18 - Avoid PEM Serialization for PyJWT Hot Paths

**Learning:** Serializing `cryptography` EC public keys to PEM format and passing them to `jwt.decode` introduces unnecessary overhead in performance-critical paths (like device authentication). `pyjwt` natively accepts `cryptography` key objects.
**Action:** Pass the native `ec.EllipticCurvePublicKey` object directly to `jwt.decode` to bypass redundant serialization/deserialization.
