...
## 2024-07-20 - Optimize SQLAlchemy Existence Checks
**Learning:** Using `select(func.count())` to check if a table is empty is an O(N) operation that scales poorly. Fetching full ORM models with `db.execute(select(Model)).scalars().first()` just to check existence incurs unnecessary hydration overhead.
**Action:** Use `db.scalar(select(Model.id).limit(1))` for O(1) table emptiness checks, use `db.scalar(select(Model.id))` for single-row existence checks without hydration, and use `db.scalar(...)` instead of `.execute(...).scalars().first()` for cleaner and faster single object retrieval.
