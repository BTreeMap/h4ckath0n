...
**Learning:** When checking for the existence of records (e.g., checking if a table is empty), using `select(func.count())` results in an expensive O(N) database query that scales poorly as the table grows.
**Action:** When checking for the existence of records, fetch a single identifier using `select(Model.id).limit(1)` and check if the result is `None` instead of counting all rows.
