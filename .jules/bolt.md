...
## 2026-06-16 - Optimize Empty Table Checks

**Learning:** Using `func.count()` to check if a table is empty (e.g. `select(func.count()).select_from(User)`) forces SQLAlchemy to issue a `COUNT(*)` query which can scan the entire table or index in O(N) time.
**Action:** Always use `await db.scalar(select(Model.id).limit(1))` and check for `None` when you only need to determine existence/emptiness. This is an O(1) index lookup and is significantly faster, especially as the table grows.
