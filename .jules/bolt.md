...
## 2024-09-04 - Avoid full table scans for existence checks
**Learning:** Using `func.count()` to check if a table is empty (e.g., `select(func.count()).select_from(User)`) causes a full table scan in large tables, which becomes a severe performance bottleneck.
**Action:** Prefer querying with `limit(1)` (e.g., `await db.scalar(select(Model.id).limit(1))`) and checking for `None` to make existence checks O(1).
