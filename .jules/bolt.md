...
## 2024-07-09 - Avoid PyJWT public key PEM serialization
**Learning:** When using `pyjwt` to decode JWTs with `cryptography` elliptic curve public keys (e.g., `ECAlgorithm`), serializing the key to a PEM string (`public_bytes().decode()`) before passing to `jwt.decode` introduces redundant parsing overhead and decreases CPU performance in hot paths. The native key object can and should be passed directly to `jwt.decode`.
**Action:** Always pass the native cryptography key object directly to `jwt.decode` instead of serializing it to a string.
