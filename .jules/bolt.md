...
## 2026-03-24 - Avoid Result Object Hydration Overhead
**Learning:** Using `db.execute(select(Model)).scalars().first()` allocates an intermediate `Result` object and can have slightly more overhead than `db.scalar(select(Model))`.
**Action:** Use `db.scalar(select(Model))` or `session.scalar(select(Model))` when fetching a single model instance to bypass the intermediate `Result` hydration.
