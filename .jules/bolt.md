...
## 2026-08-24 - Expensive Database Table Existence/Emptiness Checking
**Learning:** Using `select(func.count())` to check if a table is empty (or counting records just to see if one exists) is an O(N) operation in many database systems and can cause significant performance degradation on large tables. The repository explicitly advises against this.
**Action:** When checking if records exist or a table is empty, use `select(Model.id).limit(1)` and check if the result is `None` instead of `select(func.count())`.
