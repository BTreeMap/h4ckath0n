...
## 2026-06-26 - Optimize Single Row Fetches with scalar()

**Learning:** When using SQLAlchemy, `execute(stmt).scalars().first()` creates an intermediate `Result` and `ScalarResult` object before fetching the first row. This adds unnecessary memory overhead, especially when checking for existence or fetching just a single value like an ID. For checking existence, querying `select(Model.id)` is far more efficient than hydrating the full model.

**Action:** Always prefer `db.scalar(stmt)` over `db.execute(stmt).scalars().first()` when you only need a single row or value. When checking for a record's existence, do not `select(Model)`, use `select(Model.id)` combined with `scalar()` to completely bypass ORM parsing and hydration.
