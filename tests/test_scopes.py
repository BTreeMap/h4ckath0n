...
def test_parse_scopes():
    ...
    assert parse_scopes("foo,,bar,") == ["foo", "bar"]
    assert parse_scopes("foo,bar,foo") == ["foo", "bar"]  # deduplicates preserving order
    assert parse_scopes("  ,  ") == []


def test_format_scopes():
    assert format_scopes([]) == ""
    assert format_scopes(None) == ""
    assert format_scopes(["foo", "bar"]) == "foo,bar"
    assert format_scopes([" foo ", "bar "]) == "foo,bar"
    assert format_scopes(["foo", "", "bar"]) == "foo,bar"
    assert format_scopes(["foo", "bar", "foo"]) == "foo,bar"  # deduplicates preserving order
    assert format_scopes(["  ", " "]) == ""
