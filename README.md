...
The default authentication path uses passkeys (WebAuthn). See the Passkeys section in [API Routes](#api-routes) for the core flows.
...
`H4CKATH0N_PASSWORD_AUTH_ENABLED=true`. See the Password Auth section in [API Routes](#api-routes) for available endpoints.
