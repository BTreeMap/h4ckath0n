...
The default authentication path uses passkeys (WebAuthn). The core flows include registration, login, adding devices, and passkey management (all available under `/auth/passkey` and `/auth/passkeys` routes).
